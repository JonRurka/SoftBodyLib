...
public:
    ...
    struct TriangleCoord {
        const struct {
            float alpha,  //!< Barycentric coordinate
                  beta,   //!< Barycentric coordinate
                  gamma;  //!< Barycentric coordinate
        } barycentric;
        const float distance;  //!< Shortest (signed) distance to triangle plane
    };
    ...
    TriangleCoord operator() (const glm::vec3& p) const
    {
        ...
        const glm::vec3 result = m_matrix * (p - m_triangle.c);
...
private:
    ...
    void InitMatrix() const {
        ...
        const glm::vec3 u = m_triangle.u;
        const glm::vec3 v = m_triangle.v;
    ...
    mutable glm::mat3x3 m_matrix;  //!< Cached transformation matrix.
};
