 ...
 * @param nodes
 */
static bool SoftBodyLib::BackfaceCollisionTest(const float distance,
    ...
    const node_t nodes[])
{
    ...
    if (neighbour_node_ids.size() > weight) {
        for (auto id : neighbour_node_ids) {
            const auto neighbour_distance = glm::dot(normal, (nodes[id].AbsPosition - surface_point.AbsPosition));
...
void SoftBodyLib::ResolveCollisionForces(const float penetration_depth,
...
void SoftBodyLib::ResolveInterActorCollisions(const float dt, PointColDetector& interPointCD,
